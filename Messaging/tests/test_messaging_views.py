...
class ChatViewTests(TestCase):
    ...
    def test_client_chat_view_authenticated(self):
        ...
        response = self.client.get(reverse('client_chat', args=[self.conversation.id]))
        self.assertEqual(response.status_code, 200)

    def test_freelancer_chat_view_authenticated(self):
        # Probar la vista de chat para el freelancer estando autenticado
        self.client.login(username='user1', password='password1')
        response = self.client.get(reverse('freelancer_chat', args=[self.conversation.id]))
        self.assertEqual(response.status_code, 200)
    ...
    def test_check_conversation(self):
        ...
        self.client.login(username='user1', password='password1')
        response = self.client.get(reverse('check_conversation', args=[self.user2.id]))
        self.assertEqual(response.status_code, 200)
    ...
    def test_search_users(self):
        ...
        self.client.login(username='user1', password='password1')
        response = self.client.get(reverse('search_users') + '?q=user')
        self.assertEqual(response.status_code, 200)
